...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # Loss to enfoce large gaussians
        r_min = 0.5
        scaling_min_val = torch.min(torch.sigmoid(gaussians._scaling), dim=-1)[0]
        r_min_tensor = torch.FloatTensor([r_min]).expand_as(scaling_min_val).to(device='cuda')

        # L1 version
        #loss_scaling_min_val = torch.mean(torch.max(r_min_tensor - scaling_min_val, r_min_tensor*0.))
        # L2 version
        #loss_scaling_min_val = torch.sqrt(torch.mean(torch.square(torch.max(r_min_tensor - scaling_min_val, r_min_tensor*0.))))
        # no_hinge
        loss_scaling_min_val = torch.sqrt(torch.mean(torch.square(torch.ones_like(scaling_min_val) - scaling_min_val)))
        ...
        if (iteration % 500 == 0):
            ...
            print("Loss loss_scaling_min_val:", loss_scaling_min_val.cpu() * 1e2)
            print("gaussians._xyz.shape 5e-1 unhinged:", gaussians._xyz.shape)
        ...
        # loss = loss + loss_scaling_std*1e0 + loss_scaling_uniformity*1e-1
        # loss10
        loss = loss + loss_scaling_anisotropic * 1e-1 + loss_scaling_min_val * 5e-1
