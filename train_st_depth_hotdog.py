...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    gaussians.restore(torch.load("/home/dimakot55/output_data/gs_my/GT_hotdog/chkpnt30000.pth")[0], opt)
    ...
    GT_image_rgb = Image.open(f"./extra_data/lego_bw.jpg").convert("RGB").resize((800,800))
    GT_image_depth = Image.open(f"./extra_data/lego_bw.jpg").resize((800,800))
    GT_image_rgb = np.array(GT_image_rgb, dtype=np.float32) / 255. # / 2. + 0.25
    GT_image_depth = np.array(GT_image_depth, dtype=np.float32) / 255. #/ 2. + 0.25
    GT_image_depth = np.mean(GT_image_depth, axis=-1)

    print("torch.min(GT_image_depth), torch.max(GT_image_depth), torch.max(GT_image_depth):",
        np.min(GT_image_depth), np.max(GT_image_depth), np.max(GT_image_depth))
    ...
    with torch.no_grad():
        ...
        print("torch.min(image_depth_init), torch.max(image_depth_init), torch.max(image_depth_init):",
            torch.min(image_depth_init), torch.max(image_depth_init), torch.max(image_depth_init))


    with torch.no_grad():
        gaussians._scaling = gaussians._scaling*1.5
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        viewpoint_cam.R += np.random.randn(*viewpoint_cam.R.shape) * 0.#1.01
        viewpoint_cam.T += np.random.randn(*viewpoint_cam.T.shape) * 0.#1.01
        ...
        depth_GT_simple =  image_depth_init * (1. + (depth_GT - torch.mean(depth_GT)) * .25)
        ...
        if iteration % 1000 == 0 or iteration in [1, 5, 10, 30, 100]:
            save_dir = "./extra_results4_hotdog/"
            ...
            #arr = image_depth*255.
            arr = image_depth / np.max(image_depth)*255.
            ...
            image_depth = depth_GT_simple.detach().cpu().numpy()
