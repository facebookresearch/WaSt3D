...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # Version computing loss only to nearest neighbors
        loss_D_rotation = torch.mean(torch.square(D_rotation - D_rotation_target)* D_xyz_target_mask)
        loss_D_scaling = torch.mean(torch.square(D_scaling - D_scaling_target)* D_xyz_target_mask)
        ...
        D_to_shape = torch.cdist(gaussians._xyz, gaussians_shape._xyz / 5.)
        k = 20
        ...
        #loss_D_xyz = torch.mean(torch.square(D_xyz - D_pairwise))#*D_pairwise_target_mask)

        #loss = loss_D_xyz*1e0 + loss_D_to_shape * 3e1
        loss_D_rotation = loss_D_rotation * 1e0
        loss_D_scaling = loss_D_scaling * 1e0
        loss = loss_D_xyz + loss_D_rotation + loss_D_scaling + loss_D_to_shape * 3e2 # was 1e2 for v6
        #loss = loss_emd
        loss.backward()
        iter_end.record()
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                progress_bar.set_postfix({"loss_D_xyz": f"{loss_D_xyz:.{7}f}",
                                          ...
                                          "loss_D_to_shape": f"{loss_D_to_shape:.{7}f}",
                                          "loss": f"{loss:.{7}f}"})
