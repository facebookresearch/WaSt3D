...
RasterizeGaussiansCUDA(
	...
	const bool debug,
	const torch::Tensor& sampling_offsets)
{
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		sampling_offsets.contiguous().data<float>(),
	...
	const torch::Tensor& sampling_offsets)
{
  ...
  torch::Tensor dL_dcamViewDepth = torch::zeros({P, 1}, means3D.options());
  ...
  if(P != 0)
  {
	  ...
	  dL_dcamViewDepth.contiguous().data<float>(),
	  debug,
	  sampling_offsets.contiguous().data<float>());
  }
