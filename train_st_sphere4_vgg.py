...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    print(f"\nInitialized {len(gaussians._xyz)}, gaussians.\n")
    ...
    GT_image_rgb = np.array(GT_image_rgb, dtype=np.float32) / 255.
    GT_image_depth = np.array(GT_image_depth, dtype=np.float32) / 255.



    image_GT = torch.tensor(GT_image_rgb, dtype=torch.float32, device="cuda").permute(2, 0, 1)
    depth_GT = torch.tensor(GT_image_depth, dtype=torch.float32, device="cuda").permute(2, 0, 1)
    with torch.no_grad():
        image_GT_feats = get_features(input_tensor=image_GT, model=vgg)
        # depth_GT_feats = get_features(input_tensor=torch.stack([depth_GT]*3, axis=0), model=vgg)
        depth_GT_feats = get_features(input_tensor=depth_GT, model=vgg) # added higher depth for lego scale.
    ...
    test_simple_NST = False
    if test_simple_NST:
        #########################################################################################################
        #########################################################################################################
        # test simple optimization
        generated_image = 0.5 + torch.randn_like(image_GT) * 0.5
        generated_image = generated_image.to("cuda", torch.float).requires_grad_(True)
        optimizer = torch.optim.Adam([generated_image], lr=1e-2)
        #########################################################################################################
        #########################################################################################################
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # viewpoint_cam.R += np.random.randn(*viewpoint_cam.R.shape) * 0.1
        # viewpoint_cam.T += np.random.randn(*viewpoint_cam.T.shape) * 0.1
        ...
        #depth_GT_simple =  image_depth_init * (1. + (depth_GT - torch.mean(depth_GT)) / 2.)
        ...
        # Define losses. First extract features from the depth and rgb images
        image_rgb_feats = get_features(input_tensor=image, model=vgg)
        image_depth_feats = get_features(input_tensor=torch.stack([image_depth]*3, axis=0), model=vgg)
        # Compute losses
        layers_style = [0,1,2,3]
        layers_style = [2,3]
        layers_style = [0,1,2,3,4]
        layers_content = [1,2,3] #[2,3]
        l_style_rgb = style_loss([image_rgb_feats[i] for i in layers_style], [image_GT_feats[i] for i in layers_style])
        l_style_depth = style_loss([image_depth_feats[i] for i in layers_style], [depth_GT_feats[i] for i in layers_style])
        l_style_depth = torch.mean(image_depth) * 1e7
        l_tv = tv_loss(image)
        l_tv_depth = tv_loss(image_depth)
        # Weight losses
        l_style_rgb = l_style_rgb * 1e-6 #1e0
        l_style_depth = l_style_depth * 1e-7
        #  1e0
        l_tv = l_tv * 1e0 #1e0
        l_tv_depth = l_tv_depth * 1e0#  1e3
        loss = l_style_rgb + l_style_depth*1. + l_tv + l_tv_depth*1.
        ...
        if test_simple_NST:
            ...
            generated_image_feats = get_features(input_tensor=generated_image, model=vgg)

            l_style_gen=style_loss([generated_image_feats[i] for i in layers_style], [image_GT_feats[i] for i in layers_style])
            l_style_gen *= 1e-6
            l_tv_gen=tv_loss(generated_image)
            total_loss = l_style_gen + l_tv_gen
            ...
            if iteration % 500 == 1:
                print(f"Iteration {iteration} Losses: total_loss:{total_loss}, l_style_gen:{l_style_gen}, l_tv_gen:{l_tv_gen}")
                save_dir = "./extra_results_generated_image/"
                os.makedirs(save_dir, exist_ok=True)
                image_rgb = generated_image.detach().cpu().numpy()
                image_rgb = Image.fromarray((np.clip(np.transpose(image_rgb, (1,2,0)),0.,1.)*255.).astype(np.uint8))
                image_rgb.save(f"{save_dir}/generated_image_{iteration}_rgb.jpg")

            # Finished the optimization step of the generated image
            #########################################################################################################
            #########################################################################################################
        ...
        if (iteration % 1000 == 0) and (not test_simple_NST):
            save_dir = "./extra_results3/"
            os.makedirs(save_dir, exist_ok=True)
            # Predicition
            image_rgb = image.detach().cpu().numpy()
            image_depth = image_depth.detach().cpu().numpy()

            #np.savez(f'{save_dir}/image_depth_lego_{iteration}_pred.npz', image=image_rgb, depth=image_depth)
            image_rgb = Image.fromarray((np.clip(np.transpose(image_rgb, (1,2,0)),0.,1.)*255.).astype(np.uint8))
            image_rgb.save(f"{save_dir}/image_depth_lego_{iteration}_rgb_pred.jpg")
            ...
            image_depth.save(f"{save_dir}/image_depth_lego_{iteration}_depth_pred.jpg")



            # GT
            image_rgb = image_GT.detach().cpu().numpy()
            image_depth = depth_GT.detach().permute(1,2,0).cpu().numpy()

            #np.savez(f'{save_dir}/image_depth_lego_{iteration}_GT.npz', image=image_rgb, depth=image_depth)
            image_rgb = Image.fromarray((np.clip(np.transpose(image_rgb, (1,2,0)),0.,1.)*255.).astype(np.uint8))
            image_rgb.save(f"{save_dir}/image_depth_lego_{iteration}_rgb_GT.jpg")
            #image_depth = Image.fromarray(image_depth, mode="L")
            arr = (image_depth-np.min(image_depth))/(np.max(image_depth)-np.min(image_depth))*255.
            image_depth = Image.fromarray(arr.astype(np.uint8))
            image_depth.save(f"{save_dir}/image_depth_lego_{iteration}_depth_GT.jpg")
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                progress_bar.set_postfix({"tv": f"{l_tv:.{7}f}",
                                          ...
                                          "l_style_rgb": f"{l_style_rgb:.{7}f}",
                                          "l_style_depth": f"{l_style_depth:.{7}f}",
                                          "loss": f"{loss:.{7}f}"})
