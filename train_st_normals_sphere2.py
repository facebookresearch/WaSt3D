...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    gaussians_GT = GaussianModel(dataset.sh_degree)
    scene_GT = Scene(dataset, gaussians_GT, shuffle=True)
    gaussians_GT.restore(torch.load("/home/dimakot55/output_data/gs_my/GT_lego/chkpnt30000.pth")[0], opt)
    ...
    with torch.no_grad():
        ...
        # gaussians._scaling = gaussians._scaling*2.1
    ...
    # render GT rgb image and depth
    with torch.no_grad():
        render_pkg_GT = render(viewpoint_cam, gaussians_GT, pipe, background)
        image_GT, viewspace_point_tensor, visibility_filter, radii, depth= render_pkg_GT["render"], render_pkg_GT["viewspace_points"], render_pkg_GT["visibility_filter"], render_pkg_GT["radii"], render_pkg_GT["depth"]
        depth_GT = render_pkg_GT["depth"]

    image_GT=image_GT.clone().detach()
    depth_GT=depth_GT.clone().detach()
    gaussians_GT._xyz.requires_grad_(False)

    del gaussians_GT, scene_GT, render_pkg_GT
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        l_tv = tv_loss(image)
        l_tv_depth = tv_loss(invert_val(image_depth))

        l_rgb = l_rgb * 1. #1e0
        l_depth = l_depth * 1.#  1e0
        l_tv = l_tv * 1. #1e0
        l_tv_depth = l_tv_depth * 1.#  1e3
        loss = l_rgb + l_tv# + l_depth + l_tv_depth
